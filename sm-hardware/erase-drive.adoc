...
====
. No menu de contexto, selecione *Apagar drives*.
+
...
. Confirme a operação digitando `erase`, e depois clique em *Apagar unidades*.
+
[CAUTION]
====
...
.Resultados
A operação de apagamento pode levar vários minutos ou várias horas. Você pode visualizar o status em menu:Storage[Operations in Progress]. Quando a operação de apagamento for concluída, as unidades estarão disponíveis para uso em outro grupo de volume ou pool de discos, ou em outro storage array.
