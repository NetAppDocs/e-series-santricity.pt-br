...
.Passos
. No painel esquerdo, selecione *Home*.
. Clique no ícone do *Sino* e depois clique no link rotulado *Recuperar de _n_ problemas* no centro superior da janela.
+
...
+
O arquivo é salvo na pasta Downloads do seu navegador com o nome  `recoveryGuru-failureName-arrayName-yyyy-mm-dd-hh-mm-ss.html`.
