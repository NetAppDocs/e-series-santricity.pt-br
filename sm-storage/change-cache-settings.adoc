...
. Clique no ícone de menu: reticências [Change volume cache settings].
+
A janela Alterar configurações do cache de volume é exibida. Todos os volumes no array de storage aparecem nesta caixa de diálogo.

. Marque a caixa ao lado de cada volume para o qual deseja alterar as configurações de cache ou marque a caixa no cabeçalho da tabela para selecionar todos os volumes.
+
[cols="25h,~"]
|===
...
Para um volume fino, a pré-busca de leitura de cache dinâmico é sempre desativada e não pode ser alterada.
 ...
 a| 
Cache de gravação sem baterias
 a| 
