...
. Clique no ícone de menu:ellipsis [Alterar configurações de verificação de mídia de volume].
+
A janela Alterar configurações de verificação de mídia da unidade é exibida. Todos os volumes no array de storage aparecem nesta caixa de diálogo.

. Para ativar a verificação de mídia, selecione a caixa de seleção *Media scan*.
+
