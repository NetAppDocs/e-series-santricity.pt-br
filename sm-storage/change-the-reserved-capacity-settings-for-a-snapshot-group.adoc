...
. Selecione o grupo de snapshot que você deseja editar e, em seguida, clique no menu:ícone de reticências[Editar configurações].
+
A janela Edit snapshot group settings é exibida.
...
O nome do grupo instantâneo. É necessário especificar um nome para o grupo de instantâneos.
 ...
 a| 
Exclusão automática de Snapshot image
 a| 
