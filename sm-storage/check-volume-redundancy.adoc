...
NOTE: O recurso Verificar Redundância de Volume está atualmente disponível apenas para operações na interface Legacy System Manager.

. No painel esquerdo, selecione menu:Storage[Pools and volume groups].
. Clique no ícone de menu:reticências [Check volume redundancy] para o pool ou grupo de volume que deseja verificar.
+
