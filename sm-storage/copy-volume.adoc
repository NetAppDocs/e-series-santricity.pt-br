...
. Selecione o volume que você deseja usar como fonte para a operação de Copy Volume e, em seguida, clique no menu:ellipsis icon[Copy volume].
+
A janela Copiar Volume é exibida.
...
.Depois de terminar
Selecione menu:Storage[Operações em Andamento] para visualizar o progresso da operação de Copy Volume. Esta operação pode ser demorada e pode afetar o desempenho do sistema.
