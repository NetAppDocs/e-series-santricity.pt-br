...
* Quando é detectada capacidade não atribuída, a recomendação de Pool Auto-Configuration aparece na área de Notificações (ícone de sino na parte superior da janela). Clique em *View Pool Auto-Configuration* para abrir a caixa de diálogo.
...
. Clique no ícone de menu:reticências [Launch pool auto-configuration].
+
...
. Para visualizar características adicionais do pool, clique no nome do pool exibido.
+
