...
DA é ativado se todas as unidades forem compatíveis com DA. DA pode ser desativado após a criação do volume, selecionando menu:Storage[Volumes > ícone de reticências > Disable Data Assurance].



 a| 
Resource Provisioning Capable (somente EF80, EF50, EF300 e EF600)
 a| 
Mostra se o Resource Provisioning está disponível para este pool candidato. Resource Provisioning é um recurso disponível nos arrays de storage EF80, EF50, EF300 e EF600, que permite que os volumes sejam colocados em uso imediatamente, sem processo de inicialização em segundo plano.
