...
+
A página Create Volumes é exibida.
...
. Clique em *seguinte*.
. Se sua carga de trabalho estiver associada a um tipo de aplicativo compatível, insira as informações solicitadas; caso contrário, vá para <<Etapa 3: adicionar volumes>>.




== Etapa 3: adicionar volumes
...
+
** 2,048 volumes (EF80, EF600, EF600C e E5700 series)
** 1.024 volumes (EF50, EF300 e EF300C)
...
DA aumenta a integridade dos dados em todo o sistema de storage. O DA permite que o storage array verifique se há erros que possam ocorrer à medida que os dados são transferidos através dos controladores para as unidades. O uso DA para o novo volume garante que quaisquer erros sejam detetados.
 ...
 a| 
Recurso provisionado (EF80, EF50, EF300 e EF600 somente)
 a| 
*Sim* aparece ao lado de "Resource provisioned" somente se as unidades suportarem essa opção. Resource Provisioning é um recurso disponível nos arrays de storage EF80, EF50, EF300 e EF600, que permite que os volumes sejam colocados em uso imediatamente, sem processo de inicialização em segundo plano.

|===
====
** *Carga de trabalho específica do aplicativo* -- Clique em *Editar volumes* para alterar, adicionar ou excluir os volumes e características recomendados pelo sistema para a carga de trabalho selecionada.
+
.Detalhes do campo
[%collapsible]
====
...
Tipo de volume indica o tipo de volume que foi criado para uma carga de trabalho específica do aplicativo.
 ...
 a| 
Tamanho do bloco de volume (EF80, EF50, EF300 e EF600 somente)
 a| 
Mostra os tamanhos de bloco que podem ser criados para o volume:
...
Recurso provisionado (EF80, EF50, EF300 e EF600 somente)
 a| 
*Sim* aparece ao lado de "Recurso Provisionado" somente se as unidades suportarem essa opção. Resource Provisioning é um recurso disponível nos arrays de storage EF80, EF50, EF300 e EF600, que permite que os volumes sejam colocados em uso imediatamente, sem processo de inicialização em segundo plano.
...
+
O SMcli está disponível diretamente através do SANtricity System Manager. Esta versão para download do SMcli está disponível nos controladores EF80, EF50, E4000, EF600, EF600C, EF300, EF300C, E5700, EF570, E2800 e EF280. Para baixar o SMcli no SANtricity System Manager, selecione *Configurações* > *Sistema* e *Complementos* > *Command Line Interface*.
