...
+
Para criar um pool manualmente: selecione *Armazenamento* e, em seguida, selecione *Pools e grupos de volumes*. Na guia All Capacity, clique em menu:Create[Pool].
...
+
Para adicionar unidades manualmente a um pool existente: na página Pools e grupos de volumes, selecione o pool e clique em *Adicionar capacidade*.
...
Você pode alterar a capacidade de preservação em *Pools e grupos de volumes*. Selecione o pool que você deseja editar. Clique no menu:ícone de reticências[Editar configurações] e, em seguida, selecione a guia *Configurações*.
...
Na caixa de diálogo Adicionar capacidade, nem todas as unidades estão disponíveis para adicionar capacidade a um pool ou volume group.
...
** Você precisa ter um número par de unidades no grupo de volume. Se você não tiver um número par de unidades e ainda tiver algumas unidades não atribuídas, acesse *Pools e grupos de volumes* para adicionar unidades adicionais ao grupo de volume e tente a operação novamente.
...
Para visualizar as estatísticas do SSD Cache, selecione menu:Storage[Pools and volume groups] no painel esquerdo. Selecione o SSD Cache para o qual deseja visualizar as estatísticas e clique no ícone menu:ellipsis[View statistics]. As estatísticas nominais são exibidas na página View Statistics.
...
[NOTE]
====
O controle deslizante de Capacidade de Otimização Adicional está disponível apenas para EF80, EF50, EF600 e EF300 storage systems.
...
O provisionamento de recursos é uma funcionalidade disponível nos storage arrays EF80, EF50, EF300 e EF600, que permite que os volumes sejam colocados em uso imediatamente sem processo de inicialização em segundo plano.
...
O provisionamento de recursos está ativado por padrão em sistemas onde as unidades suportam DULBE. Você pode desativar essa configuração padrão em *Pools e grupos de volume*.
