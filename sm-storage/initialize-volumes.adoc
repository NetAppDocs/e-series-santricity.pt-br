...
. Clique no ícone de menu: reticências [Initialize volumes].
+
...
Selecione menu:Storage[Operations in Progress] para visualizar o progresso da operação de inicialização que está sendo executada para o volume selecionado. Essa operação pode ser demorada e pode afetar o desempenho do sistema.
