...
Você pode permitir que System Manager crie pools automaticamente quando detectar capacidade não alocada em um array de storage. Como alternativa, quando a criação automática não conseguir determinar a melhor configuração, você pode criar pools manualmente em menu:Storage[Pools and volume groups].
...
Você pode criar grupos de volume a partir do menu:Storage[Pools and volume groups].
